...
function BlackJack({ loggedUser, setLoggedUser }) {
    const deckOfCards = [
      "AH",
      "2H",
      "3H",
    ...
    const [chipCount, setChipCount] = useState(loggedUser.balance);
    ...
    const startHand = () => {
      if (isHandComplete && lockedBet > 0) {
        ...
        fetch('https://mysite-8r0y.onrender.com/blackjackstart', {
            ...
            body: JSON.stringify({balance: lockedBet})
        })
        .then(res => res.json())
        .then(data => {
            setLoggedUser(data)
    ...
    const keepSameBet = () => {
      if (isHandComplete && chipCount >= previousBet && previousBet > 0) {
        ...
        fetch('https://mysite-8r0y.onrender.com/blackjackstart', {
            ...
            body: JSON.stringify({balance: previousBet})
        })
        .then(res => res.json())
        .then(data => {
            setLoggedUser(data)
    ...
    useEffect(() => {
      ...
      if (winner === "player" && !isBlackjack) {
        if (didDouble) {
            fetch('https://mysite-8r0y.onrender.com/blackjackfinish', {
                ...
                body: JSON.stringify({balance: previousBet * 4})
            })
            .then(res => res.json())
            .then(data => {
                setLoggedUser(data)
          ...
          setChipCount(chipCount + previousBet * 4);
            ...
            fetch('https://mysite-8r0y.onrender.com/blackjackfinish', {
                ...
                body: JSON.stringify({balance: previousBet * 2})
            })
            .then(res => res.json())
            .then(data => {
                setLoggedUser(data)
      ...
      if (winner === "player" && isBlackjack) {
        fetch('https://mysite-8r0y.onrender.com/blackjackfinish', {
            ...
            body: JSON.stringify({balance: previousBet * 2.5})
        })
        .then(res => res.json())
        .then(data => {
            setLoggedUser(data)
    ...
    <div className="app">
      <header className="app-header">
        ...
        <p className="chip-count">
          ...
          >
            <span id={chipCount === 0 ? 'attention-red' : 'greeny'}>$</span>{chipCount}
          </div>
