...
function Roulette({ loggedUser, setLoggedUser }) {
    ...
    const resetLayout = () => {
      ...
      if (!isSpinComplete) {
        fetch('https://mysite-8r0y.onrender.com/blackjackfinish', {
            ...
            body: JSON.stringify({balance: pendingTotalBet})
        })
        .then(res => res.json())
        .then(data => {
          setLoggedUser(data)
    ...
    const undoRecentBet = () => {
      ...
      fetch('https://mysite-8r0y.onrender.com/blackjackfinish', {
        ...
        body: JSON.stringify({balance: betValue})
    })
    .then(res => res.json())
    .then(data => {
        setLoggedUser(data)
    ...
    const collectWinnings = () => {
      ...
      if (totalAmountWon > 0) {
        fetch('https://mysite-8r0y.onrender.com/blackjackfinish', {
            ...
            body: JSON.stringify({balance: totalAmountWon})
        })
        .then(res => res.json())
        .then(data => {
            setLoggedUser(data)
